...
export const Card = ({
  ...
  let _authorHref = `https://www.npmjs.com/~${author}`;
  let _package = `${name}${version ? "@" + version : ""}`;
  let _packageHref = `https://www.npmjs.com/package/${name}`;
  let _packageVersionHref = `${_packageHref}/v/${version}`;
  ...
  return (
    <div class="card">
      <section class="content">
        <h2 class="font-semibold text-lg">
          <a href={version ? _packageVersionHref : _packageHref} target="_blank">
            {_package}
          </a>
        </h2>
        <p>{description}</p>
...
export const SearchResults = () => {
  return (
    <div class={`search-results`}>
      <Show when={getState().length == 0}>
        <ErrorCard name="No results..." description=""></ErrorCard>
      </Show>
      <For each={getState()}>
        {({ name, description, version, author, date, type }) => {
          return (
            <Show
              when={type !== "error"}
              fallback={<ErrorCard name={name} description={description} />}
            >
              <Card
                name={name}
