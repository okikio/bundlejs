...
export class UntarStream
  ...
  constructor() {
    this.#gen = async function* () {
      ...
      const stream = readable.pipeThrough(new FixedChunkStream(512));
    ...
    const { readable, writable } = new TransformStream<
      ...
      Uint8Array
    >();
    this.#readable = readableStreamFrom(this.#untar());
...
/**
 * Polyfill for ReadableStream.from() which converts an (async) iterable into a ReadableStream.
 *
 * @param source - The iterable or async iterable to convert into a ReadableStream
 * @returns A new ReadableStream created from the source iterable
 */
function readableStreamFrom<T>(source: Iterable<T> | AsyncIterable<T>): ReadableStream<T> {
  return new ReadableStream<T>({
    async start(controller) {
      ...
      // Check if the source is an async iterable or a synchronous iterable
      while (true) {
