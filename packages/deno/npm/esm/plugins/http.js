import { getRequest } from '../utils/fetch-and-cache.js';
import { decode } from '../utils/encode-decode.js';
import { getCDNUrl, DEFAULT_CDN_HOST, getCDNStyle } from '../utils/util-cdn.js';
import { inferLoader } from '../utils/loader.js';
import { urlJoin, extname, isBareImport } from "../utils/path.js";
import { CDN_RESOLVE } from './cdn.js';
 ...
 */
export const fetchPkg = async (url, events) => {
    try {
 ...
 * @param logger Console log
 */
export const fetchAssets = async (path, content, namespace, events, config) => {
 ...
 * @param logger Console log
 */
export const HTTP_RESOLVE = (host = DEFAULT_CDN_HOST, events) => {
    return async (args) => {
        ...
        if (!argPath.startsWith(".")) {
            ...
            urlJoin(args.pluginData?.url ? args.pluginData?.url : host, "../", argPath)).origin;
            ...
            if (isBareImport(argPath)) {
                return await CDN_RESOLVE(origin, events)(args);
            }
            else {
                /**
 ...
 * @param logger Console log
 */
export const HTTP = (events, state, config) => {
    ...
    return {
        ...
        setup(build) {
            ...
            build.onLoad({ filter: /.*/, namespace: HTTP_NAMESPACE }, async (args) => {
                ...
                const _assetResults = (await fetchAssets(url, content, args.namespace, events, config))
                    .filter((result) => {
