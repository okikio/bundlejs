...
export async function getESBUILD(platform = "node") {
    try {
        switch (platform) {
            case "node":
...
export async function build(opts = {}) {
    ...
    try {
        ...
        outputs = await Promise.all([...STATE.assets]
        ...
        const totalByteLength = bytes(content.reduce((acc, { byteLength }) => acc + byteLength, 0));
        const totalCompressedSize = bytes((await Promise.all(content.map((code) => {
            switch (type) {
                case "lz4":
