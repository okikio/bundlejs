/** Based on https://github.com/hardfist/neo-tools/blob/main/packages/bundler/src/plugins/unpkg.ts */
...
import { getCDNUrl, getCDNStyle } from '../utils/util-cdn.js';
...
import { HTTP_NAMESPACE } from './http.js';
    ...
    if (isBareImport(args.path)) {
      ...
      if (NPM_CDN) {
        try {
          ...
          const path = resolve(pkg, subpath ? "." + subpath.replace(/^\.?\/?/, "/") : ".", {
            require: args.kind === "require-call" || args.kind === "require-resolve",
          }) || legacy(pkg);
 ...
 * @param logger Console log
 */
export const CDN = (events: typeof EVENTS, _state: typeof STATE, config: BundleConfigOptions): Plugin => {
