import { source as on } from "./esbuild-wasm-1ddcba4c.mjs";
const Ke = "0.15.9", le = "Deno" in globalThis ? "deno" : "process" in globalThis ? "node" : "browser";
var at = class {
  constructor(e) {
    this.map = new Map(e);
  }
...
async function ts(e = le) {
  try {
    switch (e) {
      ...
      default:
        return await import("./esbuild-05e60b75.mjs").then((t) => t.b);
    }
  } catch (t) {
    throw t;
  }
    ...
    if (!Y("initialized")) {
      ...
      if (_e("esbuild", s), e !== "node" && e !== "deno")
        if ("wasmModule" in t)
          ...
          const { default: r } = await import("./esbuild-wasm-1ddcba4c.mjs");
          await s.initialize({
...
const ie = async (e, t = "error", s = !0) => {
  const { formatMessages: r } = await import("./esbuild-05e60b75.mjs").then((i) => i.b);
...
const Ze = async () => {
  if (Se)
    return Se;
  const e = await import("./brotli-e0e80880.mjs"), { default: t, source: s } = e;
