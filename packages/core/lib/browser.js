...
`+t.slice(1).join(`
`),suggestion:""}}break}}return null}function Se(e,t,r){let u=5,p=t.length<1?"":` with ${t.length} error${t.length<2?"":"s"}:`+t.slice(0,u+1).map((i,y)=>{if(y===u)return`
...`;if(!i.location)return`
error: ${i.text}`;let{file:a,line:T,column:B}=i.location,O=i.pluginName?`[plugin: ${i.pluginName}] `:"";return`
${a}:${T}:${B}: ERROR: ${O}${i.text}`}).join(""),w=new Error(`${e}${p}`);return w.errors=t,w.warnings=r,w}function ye(e,t){for(const r of e)r.detail=t.load(r.detail);return e}function rt(e,t){if(e==null)return null;let r={},u=l(e,r,"file",x),p=l(e,r,"namespace",x),w=l(e,r,"line",Ee),i=l(e,r,"column",Ee),y=l(e,r,"length",Ee),a=l(e,r,"lineText",x),T=l(e,r,"suggestion",x);return se(e,r,t),{file:u||"",namespace:p||"",line:w||0,column:i||0,length:y||0,lineText:a||"",suggestion:T||""}}function _e(e,t,r,u){let p=[],w=0;for(const i of e){let y={},a=l(i,y,"id",x),T=l(i,y,"pluginName",x),B=l(i,y,"text",x),O=l(i,y,"location",Xe),m=l(i,y,"notes",re),L=l(i,y,"detail",Pe),Q=`in element ${w} of "${t}"`;se(i,y,Q);let C=[];if(m)for(const g of m){let d={},v=l(g,d,"text",x),S=l(g,d,"location",Xe);se(g,d,Q),C.push({text:v||"",location:rt(S,Q)})}p.push({id:a||"",pluginName:T||u,text:B||"",location:rt(O,Q),notes:C,detail:r?r.store(L):-1}),w++}return p}function Ae(e,t){const r=[];for(const u of e){if(typeof u!="string")throw new Error(`${JSON.stringify(t)} must be an array of strings`);r.push(u)}return r}function _t({path:e,contents:t}){let r=null;return{path:e,contents:t,get text(){return r===null&&(r=ke(t)),r}}}var xt="0.14.46",kt=e=>Ue().build(e),Et=()=>{throw new Error('The "serve" API only works in node')},Ot=(e,t)=>Ue().transform(e,t),St=(e,t)=>Ue().formatMessages(e,t),jt=(e,t)=>Ue().analyzeMetafile(e,t),Tt=()=>{throw new Error('The "buildSync" API only works in node')},$t=()=>{throw new Error('The "transformSync" API only works in node')},Pt=()=>{throw new Error('The "formatMessagesSync" API only works in node')},Rt=()=>{throw new Error('The "analyzeMetafileSync" API only works in node')},je,Fe,Ue=()=>{if(Fe)return Fe;throw je?new Error('You need to wait for the promise returned from "initialize" to be resolved before calling this'):new Error('You need to call "initialize" before calling this')},Dt=e=>{e=wt(e||{});let t=e.wasmURL,r=e.wasmModule,u=e.worker!==!1;if(!t&&!r)throw new Error('Must provide either the "wasmURL" option or the "wasmModule" option');if(je)throw new Error('Cannot call "initialize" more than once');return je=Ct(t||"",r,u),je.catch(()=>{je=void 0}),je},Ct=(e,t,r)=>xe(void 0,null,function*(){let u;if(t)u=t;else{let y=yield fetch(e);if(!y.ok)throw new Error(`Failed to download ${JSON.stringify(e)}`);u=yield y.arrayBuffer()}let p;if(r){let y=new Blob([`onmessage=((postMessage) => {
