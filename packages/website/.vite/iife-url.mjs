...
const rawRE = /(\?|&)raw(?:&|$)/;
const urlRE = /(\?|&)iife-url(?:&|$)/;
const assetCache = new Map();
const assetHashToFilenameMap = new Map();
// save hashes of the files that has been emitted in build watch
const emittedHashMap = new Map();
/**
 * Also supports loading plain strings with import text from './foo.txt?raw'
 */
export function IIFE_URL(config) {
    ...
    return {
        ...
        async renderChunk(code, chunk) {
          ...
          if (s) {
                let result = await transformWithEsbuild(s.toString(), filename, {
                    format: "iife"
                });
                return {
                    code: result.code,
...
export function getAssetFilename(hash, config) {
 ...
 */
export function assetFileNamesToFileName(assetFileNames, file, contentHash, content) {
...
export function getAssetHash(content) {
    return createHash('sha256').update(content).digest('hex').slice(0, 8);
}
export async function urlToBuiltUrl(url, importer, config, pluginContext) {
